...
void QuineMcCluskey::buildTables(const set<int> &minterms) {
  this->groups.clear();
  this->groups.resize(this->vars_num + 1);

  for (const auto &term : minterms) {
    size_t term_high_literals = 0;
    string term_literals_vars(vars_num, '\0');
...
QuineMcCluskey::groups_t QuineMcCluskey::generate() {
  vector<groups_t> all_groups(this->vars_num);
      ...
      for (auto &low_table : low_group) {
        for (auto &high_table : high_group) {
          ...
          if (diff == 1) {
            ...
            all_groups[i + 1][getHighLiteralsNum(new_table.literals)].push_back(
                new_table);
...
size_t QuineMcCluskey::getHighLiteralsNum(const string &literals) {
  size_t num = 0;
